...
if (/^https?:\/\/(?:meta.)?codegolf.stackexchange.com/.test(window.location)) {
  if (site == "main") {
    ...
    //qS("#hlogo > a").innerHTML = "<table id=\"newlogo\"><tr><td><img src=\"" + meta.FAVICON + "\" height=60></td><td>Programming Puzzles &amp; Code Golf <span class=\"meta-title\">meta</span></td></tr></table>";
  }
  ...
  document.head.innerHTML += ("<style>@import url(" + FONT_URL + ");" +
    ...
    "a.post-tag{border-radius: 0;font-family:"+MONOSPACE_FONT+";font-size:12px;white-space: nowrap;background-color:$$TAG_COLOR;border-color:$$TAG_BORDER_COLOR; -webkit-transition: color 0.08s ease, background 0.08s ease, border-color 0.08s ease; -moz-transition: color 0.08s ease, background 0.08s ease, border-color 0.08s ease; -ms-transition: color 0.08s ease, background 0.08s ease, border-color 0.08s ease; -o-transition: color 0.08s ease, background 0.08s ease, border-color 0.08s ease}" +
